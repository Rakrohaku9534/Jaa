...
const { BufferJSON, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, proto, generateWAMessageContent, generateWAMessage, prepareWAMessageMedia, areJidsSameUser, getContentType, isBaileys } = require("@whiskeysockets/baileys");
const fs = require("fs");
...
const thumb = fs.readFileSync ('./comot/jangandiubah/image/thumb.jpg')
...
const senderr = m.key.fromMe ? (comot.user.id.split(':')[0]+'@s.whatsapp.net' || comot.user.id) : (m.key.participant || m.key.remoteJid)
...
let mute = JSON.parse(fs.readFileSync('./comot/database/mute.json'));
...
𝗠𝗲𝗻𝘂
♂️${prefix}ʟᴀᴘᴏʀᴀɴᴍᴇɴᴜ
♂️${prefix}ᴅᴏᴡɴʟᴏᴀᴅᴍᴇɴᴜ
♂️‎${prefix}ɢʀᴏᴜᴘᴍᴇɴᴜ
♂️${prefix}ᴘʀᴇᴍɪᴜᴍᴍᴇɴᴜ
♂️‎${prefix}ᴄᴏɴᴠᴇʀᴛᴍᴇɴᴜ
♂️${prefix}ʀᴀɴᴅᴏᴍᴍᴇɴᴜ
♂️${prefix}ᴀsᴜᴘᴀɴᴍᴇɴᴜ
♂️‎${prefix}ᴏᴡɴᴇʀᴍᴇɴᴜ

𝗖𝗿𝗲𝗮𝘁𝗲𝗱 © 𝟮𝟬𝟮𝟯 𝗖𝗼𝗺𝗼𝘁 𝗠𝗗`
comot.relayMessage(m.chat,  {
...
break
case 'tomp4': {
if (!/webp/.test(mime)) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : 𝗿𝗲𝗽𝗹𝘆 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 ${prefix + command}`)
reply(mess.wait)
...
break
case 'toaud': {
if (!/video/.test(mime) && !/audio/.test(mime)) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : 𝗿𝗲𝗽𝗹𝘆 𝘃𝗶𝗱𝗲𝗼/𝗮𝘂𝗱𝗶𝗼 ${prefix + command}`)
reply(mess.wait)
...
const {
quote
} = require('./comot/database/quote.js')
if (!q) return reply('𝗠𝗮𝘀𝘂𝗸𝗮𝗻 𝘁𝗲𝘅𝘁')
let ppnyauser = await await comot.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/6880771a42bad09dd6087.jpg')
const rest = await quote(q, pushname, ppnyauser)
reply(mess.wait)
comot.sendImageAsSticker(m.chat, rest.result, m, {
...
if (args[0] === "on") {
if (isMute) return reply(`𝗨𝗱𝗮𝗵 𝗺𝘂𝘁𝗲`)
mute.push(m.chat)
fs.writeFileSync('./comot/database/mute.json', JSON.stringify(mute, null, 2))
reply('𝗕𝗲𝗿𝗵𝗮𝘀𝗶𝗹 𝗺𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗺𝘂𝘁𝗲 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽')
}
else if (args[0] === "off") {
if (!isMute) return reply(`𝗨𝗱𝗮𝗵 𝘂𝗻𝗺𝘂𝘁𝗲`)
let anu = mute.indexOf(m.chat)
mute.splice(anu, 1)
fs.writeFileSync('./comot/database/mute.json', JSON.stringify(mute, null, 2))
reply('𝗕𝗲𝗿𝗵𝗮𝘀𝗶𝗹 𝗺𝗲𝗻𝗼𝗻𝗮𝘁𝗶𝗳𝗸𝗮𝗻 𝗺𝘂𝘁𝗲 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽')
}else {
reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : ${prefix+command} 𝗼𝗻/𝗼𝗳𝗳`)
}}
break
case'xnxxsearch':{
if (!text) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : ${prefix+command} 𝗷𝗮𝗽𝗮𝗻𝘀𝗲`)
reply(mess.wait)
let anu = await fetchJson(`https://xzn.wtf/api/xnxxsearch?title=${text}&apikey=fahrul12`)
let teks = ``
for (let v of anu) {
teks += `*Title* : ${v.title}\n`
...
reply(mess.wait)
let anu = await fetchJson(`https://xzn.wtf/api/xnxxdl?urlxnxx=${text}&apikey=fahrul12`)
const cpt = anu.title
const nick = anu.quality
comot.sendMessage(m.chat, { video: { url: anu.url}, caption: `Judul : ${cpt}\nQuality : ${nick}`}, {quoted: m})
}
break
case 'ytmp4': {
if (!text) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : ${prefix+command} *𝘂𝗿𝗹*`)
...
case 'infobansos': {
if (!isCreator) return reply(mess.owner)
if (!text) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : ${prefix + command} 𝗰𝗼𝗺𝗼𝘁-𝘅𝗱`)
var call = { 
scheduledCallCreationMessage: {
callType: 2,
...
if (args.length == 0) return reply(`𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : ${prefix + command} 𝗰𝗼𝗺𝗼𝘁 𝘅𝗱`)
ini_txt = args.join(" ")
ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/${command}?apikey=haikalgans&text=${ini_txt}`)
comot.sendMessage(from, { sticker : ini_buffer }, {
...
case 'join': {
if (!isCreator) return reply(mess.owner)
if (!text) return '𝗠𝗮𝘀𝘂𝗸𝗮𝗻 𝗹𝗶𝗻𝗸 𝗴𝗿𝗼𝘂𝗽'
if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) return '𝗟𝗶𝗻𝗸 𝗶𝗻𝘃𝗮𝗹𝗶𝗱'
reply(mess.wait)
